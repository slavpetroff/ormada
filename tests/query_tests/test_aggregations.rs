...
async fn create_test_author(db: &sea_orm::DatabaseConnection) -> author::Model {
    author::Entity::objects(db).create(author::Model {
...
async fn test_aggregations_comprehensive() {
    ...
    let sum = book::Entity::objects(db).aggregate_sum(book::Column::Price).await.unwrap();
    let avg = book::Entity::objects(db).aggregate_avg(book::Column::Price).await.unwrap();
    let max = book::Entity::objects(db).aggregate_max(book::Column::Price).await.unwrap();
    let min = book::Entity::objects(db).aggregate_min(book::Column::Price).await.unwrap();
