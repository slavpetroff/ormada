...
async fn test_delete_uses_bulk_in_clause() {
    ...
    let count = Author::objects(&db).filter(Author::Age.gte(20)).delete().await.unwrap();
...
async fn test_delete_empty_result() {
    ...
    let count = Author::objects(&db).filter(Author::Age.gt(999)).delete().await.unwrap();
...
async fn test_delete_single_record() {
    ...
    let count = Author::objects(&db).filter(Author::Id.eq(target_id)).delete().await.unwrap();
...
async fn test_update_with_select_for_update() {
    ...
    let updated = Author::objects(&db).filter(Author::Id.eq(authors[0].id)).first().await.unwrap();
