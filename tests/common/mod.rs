...
use seaorm_django::prelude::*; // Import prelude
...
pub async fn create_sample_authors(db: &DatabaseConnection) -> Vec<author::Model> {
    ...
    let author1 = author::Entity::objects(db).create(author::Model {
        name: "Alice Johnson".to_string(),
        email: "alice@example.com".to_string(),
        age: 35,
        ..Default::default()
    }).await.expect("Failed to insert author");
    authors.push(author1);

    let author2 = author::Entity::objects(db).create(author::Model {
    ...
    let author3 = author::Entity::objects(db).create(author::Model {
...
pub async fn create_sample_books(db: &DatabaseConnection) -> Vec<book::Model> {
    ...
    let book1 = book::Entity::objects(db).create(book::Model {
        title: "Rust Programming".to_string(),
        author_id: 1,
        price: 4999,  // $49.99
        published: true,
        ..Default::default()
    }).await.expect("Failed to insert book");
    books.push(book1);

    let book2 = book::Entity::objects(db).create(book::Model {
    ...
    let book3 = book::Entity::objects(db).create(book::Model {
