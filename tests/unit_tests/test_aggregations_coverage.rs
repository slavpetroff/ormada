...
async fn test_sum_on_float_column() {
    ...
    let result = Book::objects(&db).aggregate_sum(Book::Price).await.unwrap();
...
async fn test_avg_returns_float() {
    ...
    let _author1 = Author::objects(&db)
        .create(Author {
            id: 0,
    ...
    let _author2 = Author::objects(&db)
        .create(Author {
            id: 0,
